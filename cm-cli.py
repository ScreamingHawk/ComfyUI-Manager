...
def show_list(kind, simple=False):
    ...
    processed = {}
    unknown_processed = []
    if kind in ['all', 'cnr', 'installed', 'enabled']:
        ...
        if kind != 'cnr':
            for k, v in unified_manager.unknown_active_nodes.items():
                ...
                log_item = "[    ENABLED    ] ", item['title'], short_name, item['author']
                unknown_processed.append(log_item)

    if kind in ['all', 'cnr', 'installed', 'disabled']:
        ...
        if kind != 'cnr':
            for k, v in unified_manager.unknown_inactive_nodes.items():
                ...
                short_name = item.get('id', '')
                log_item = "[    DISABLED   ] ", item['title'], short_name, item['author']
                unknown_processed.append(log_item)

    if kind in ['all', 'cnr', 'not-installed']:
        ...
        if kind != 'cnr':
            for x in not_installed_unknown_nodes:
                short_name = x.get('id', '')
                log_item = "[    DISABLED   ] ", x['title'], short_name, x['author']
                unknown_processed.append(log_item)
...
def restore_snapshot(
    ...
    try:
        ...
        for url in cloned_repos:
            unified_manager.execute_install_script('', url, instant_execution=True)
